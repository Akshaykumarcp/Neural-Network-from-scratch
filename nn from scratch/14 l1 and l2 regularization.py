...
L1 and L2 regularization are used to calculate a number (called a penalty ) added to the loss value to penalize the model for large
weights and biases.
...
- This non-linear approach penalizes larger weights and biases more than smaller ones because of the
square function used to calculate the result.

Why L2 Reg is commonly used ?
- In other words, L2 regularization is commonly used as it does not affect small parameter values substantially and does not allow the model to grow
weights too large by heavily penalizing relatively big values.
...
being invariant to small inputs and variant only to the bigger ones.
- That’s why L1 regularization is rarely used alone and usually combined with L2 regularization if it’s even used at all.

How it can help exploding gradients!!
...
weights to become very large values).

lambda in reg:
we want to dictate how much of an
